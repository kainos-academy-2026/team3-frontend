...
async function fillEditForm(
	...
	const entries = Object.entries(data) as Array<
		[keyof EditableJobRoleFields, string]
	>;
...
Then(
	"I should be redirected to the job role detail page",
	async function (this: BddWorld) {
		const page = ensurePage(this);
		...
		await expect(page).toHaveURL(
			new RegExp(`/job-roles/${this.currentJobRoleId}(?:\\?.*)?$`),
		);
...
Then(
	...
	async function (this: BddWorld, roleName: string) {
		await expect(ensurePage(this).locator("#job-role-heading")).toContainText(
			roleName,
		);
	},
);
...
Then(
	"I should remain on the edit job role page",
	async function (this: BddWorld) {
