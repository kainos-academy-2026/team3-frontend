...
class CustomWorld extends World {
	...
	async apiGet(endpoint, token = null) {
		...
		this.lastResponse = await this.page.request.get(`${this.baseURL}${endpoint}`, { headers });
	...
	async apiPost(endpoint, payload, token = null) {
		if (!this.page) throw new Error('Page not initialized');
		const headers = token ? { Authorization: `Bearer ${token}` } : {};
		this.lastResponse = await this.page.request.post(`${this.baseURL}${endpoint}`, {
			data: payload,
			headers,
		});
		this.lastStatus = this.lastResponse.status();
	...
	async apiPatch(endpoint, payload, token = null) {
		if (!this.page) throw new Error('Page not initialized');
		const headers = token ? { Authorization: `Bearer ${token}` } : {};
		this.lastResponse = await this.page.request.patch(`${this.baseURL}${endpoint}`, {
			data: payload,
			headers,
		});
		this.lastStatus = this.lastResponse.status();
	...
	async clickButton(buttonPattern) {
		...
		const button = this.page.getByRole('button', { name: new RegExp(buttonPattern, 'i') });
		
		const isVisible = await button.isVisible({ timeout: 2000 }).catch(() => false);
		if (!isVisible) {
			// List available buttons for debugging
			const allButtons = await this.page.locator('button').allTextContents().catch(() => []);
			throw new Error(`Button with pattern "${buttonPattern}" not found. Available buttons: ${allButtons.join(', ')}`);
		}
	...
	async hasErrorMessage(errorText) {
		...
		return alert.filter({ hasText: errorText }).isVisible().catch(() => false);
	}
}
