...
Given('I am authenticated as an admin', { timeout: 30000 }, async function () {
	...
	const alertText = await this.page.locator('[role="alert"]').textContent().catch(() => '');
	if (alertText) {
...
Given('I am authenticated as a regular user', { timeout: 30000 }, async function () {
	await this.navigateTo('/login');
	...
	const currentUrl = this.page.url();
	const cookies = await this.context.cookies();
	console.log(`After user login - URL: ${currentUrl}, Cookies: ${cookies.map(c => c.name).join(',')}`);
});

// When steps
When('I navigate to the edit page for job role with ID {int}', async function (jobRoleId) {
  const detailUrl = `${this.baseURL}/job-roles/${jobRoleId}`;
  console.log(`Navigating to detail page first: ${detailUrl}`);

	const response = await this.page.goto(detailUrl, { waitUntil: 'domcontentloaded' });
  console.log(`Detail page response status: ${response?.status()}`);

  await this.page.getByRole('link', { name: /edit job role/i }).click();
	await this.page.waitForURL(new RegExp(`/job-roles/${jobRoleId}/edit(?:\\?.*)?$`), {
		timeout: 10000,
	});
...
When('I update all fields with valid data', { timeout: 15000 }, async function () {
	const updateData = {
	...
	const allButtons = await this.page.locator('button').allTextContents().catch(() => []);
...
When('I try to access the edit page for job role with ID {int} directly', async function (jobRoleId) {
	// Store current cookies to verify auth is still there
	const cookiesBefore = await this.context.cookies();
	console.log(`Cookies before accessing edit page: ${cookiesBefore.map(c => c.name).join(',')}`);
	console.log(`Session userRole should be: ${this.page._userRole || 'unknown'}`);
	
	const response = await this.page.goto(`${this.baseURL}/job-roles/${jobRoleId}/edit`);
	this.lastStatus = response?.status();
	
	const finalUrl = this.page.url();
	console.log(`After accessing edit page - Status: ${this.lastStatus}, Final URL: ${finalUrl}`);
});

When('I try to access the edit page for job role with ID {int} directly without authentication', async function (jobRoleId) {
...
Then('the detail page shows the updated role name {string}', { timeout: 15000 }, async function (roleName) {
